...
outpvals = 0:100;
xplev = outpvals;
load('D:\Thesis\Code\subcode\EFHdata.mat','Inhal','surface','frequency','Inhalkg');
%%
...
%   inhPctiles(i,:) = prctile(inhData(i,:),outpvals); 
  inhkgPctiles(i,:) = prctile(getPercentiles(plev.inh,[Inhalkg(i+2,4) Inhalkg(i+2,4:11)],1000),outpvals); 
end
%%
...
hands=2*[5.3 5.3 5.3 5.3 5.7 4.7 4.9  4.7 4.5 4.5 5.2 4.8 5.2 4.8 5.2 4.8 5.2]*SensitiveFactor(14)/100;
...
for i=1:17
      ...
      dermalrate(1+i,3:12)=surfaceload(1+i,3:12)*24*vd*namdav*SensitiveFactor(9)*0.001*SensitiveFactor(18);
...
for i=1:6
      ...
      ingest(1+i,3:12)=surface_hands(1+i,3:12)*frequency(i,1)*24*vd*SensitiveFactor(12)*namdav*SensitiveFactor(9)*0.01;
end
for i=6:17
       %dermal_contact(1+i,1:2)=surface_hands(1+i,1:2);
      ingest(1+i,3:12)=surface_hands(1+i,3:12)*24*vd*SensitiveFactor(12)*namdav*SensitiveFactor(9)*0.01;
...
%% time spent indoor outdoor data
