...
for z = 1:numel(ActivityList)    
    for i = 1:length(x_data)-1
        Y(i+j) = ActivityVentilation.male.(ActivityList{z,1})(i,4);
        Y(i+j+1) = ActivityVentilation.male.(ActivityList{z,1})(i,4);
        j = j + 1;
    end
    Y(end) = ActivityVentilation.male.(ActivityList{z,1})(end,4);
    h(z) = plot(X,Y);
    j = 0;
end
...
%% Title and labels
hTitle = title(sprintf('Ventilation Rate During Activity (L/min-kg)\n adjusted for Body Weight'));
...
%% Print FRC 
print(gcf, '-dpdf', '-r300', 'plots/activity_male_vent_adj');  
print(gcf, '-dpng', '-r300', 'plots/activity_male_vent_adj');  