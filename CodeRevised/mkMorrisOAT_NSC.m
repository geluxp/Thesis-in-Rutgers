...
for II=1:NumTraj
    for JJ=1:k
        ...
        DeltaInput(II,deltaV~=0)=deltaP*(UpBound-LowBound)/(NormInputP*(UpBound-LowBound)+LowBound);
    end
end        
%%
  ...
  for II=1:NumTraj
    for JJ=1:k    
        %Normalized Sensitivity Coefficients
          ...
          NSC.Inha{II,JJ}(1)=(-mean(ExposureInha{II,JJ}(1))+mean(ExposureInha{II,JJ+1}(1)))/(DeltaInput(II,JJ)*mean(ExposureInha{II,JJ}(1)));
          NSC.Derm{II,JJ}(1)=(-mean(ExposureDerm{II,JJ}(1))+mean(ExposureDerm{II,JJ+1}(1)))/(DeltaInput(II,JJ)*mean(ExposureDerm{II,JJ}(1)));
          NSC.Ingest{II,JJ}(1)=(-mean(ExposureIngest{II,JJ}(1))+mean(ExposureIngest{II,JJ+1}(1)))/(DeltaInput(II,JJ)*mean(ExposureIngest{II,JJ}(1)));
          NSC.Total{II,JJ}(1)=(-mean(ExposureTotal{II,JJ}(1))+mean(ExposureTotal{II,JJ+1}(1)))/(DeltaInput(II,JJ)*mean(ExposureTotal{II,JJ}(1))); 
     end
   end
